...
def get_script(relative_link):
    tail = relative_link.split('/')[-1] #This is the name.html
    print('fetching %s' % tail)
    script_front_url = BASE_URL + quote(relative_link) #puts % for spaces
    front_page_response = requests.get(script_front_url)
    front_soup = BeautifulSoup(front_page_response.text, "html.parser") #Full HTML

    try:
        script_link = front_soup.find_all('p', align="center")[0].a['href']#Link to script
    except IndexError:
...
if __name__ == "__main__":
    ...
    for p in paragraphs:
        relative_link = p.a['href'] #This is printing the URL
